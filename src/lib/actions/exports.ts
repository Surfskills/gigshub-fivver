...
export async function exportReportsToCSV(startDate: string, endDate: string) {
  ...
  const csvData = reports.map((report) => ({
    ...
    'Orders in Progress Value': Number(report.ordersInProgressValue ?? 0).toFixed(2),
    'Ranking Page': report.rankingPage || '',
    Rating: report.rating != null ? Number(report.rating) : '',
    'Response Rate (%)': report.responseRate != null ? Number(report.responseRate) : '',
    'Earnings to Date': report.earningsToDate != null ? Number(report.earningsToDate).toFixed(2) : '',
