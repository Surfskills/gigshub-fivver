...
interface AccountsCreatedSectionProps {
  isAdmin?: boolean;
}

export function AccountsCreatedSection({ isAdmin = false }: AccountsCreatedSectionProps) {
  const [subTab, setSubTab] = useState<SubTab>(isAdmin ? 'add' : 'view');
  ...
  return (
    <div className="space-y-4">
      ...
      {subTab === 'add' && isAdmin && <AccountsCreatedForm />}
