...
;// CONCATENATED MODULE: ./node_modules/@clerk/shared/dist/chunk-T4WHYQYX.mjs
// src/error.ts
...
var snakecase_keys = __webpack_require__(562);
...
;// CONCATENATED MODULE: ./node_modules/@clerk/shared/dist/chunk-QMOEH4QX.mjs
...
;// CONCATENATED MODULE: ./node_modules/@clerk/shared/dist/chunk-4EIZQYWK.mjs
...
;// CONCATENATED MODULE: ./node_modules/@clerk/backend/dist/chunk-HGGLOBDA.mjs
...
function buildRequest(options) {
    const requestFn = async (requestOptions)=>{
        ...
        try {
            ...
            const isJSONResponse = res?.headers && res.headers?.get(chunk_HGGLOBDA_constants.Headers.ContentType) === chunk_HGGLOBDA_constants.ContentTypes.Json;
            const responseBody = await (isJSONResponse ? res.json() : res.text());
            if (!res.ok) {
                return {
                    data: null,
                    errors: chunk_HGGLOBDA_parseErrors(responseBody),
                    status: res?.status,
            ...
            return {
                data: null,
                errors: chunk_HGGLOBDA_parseErrors(err),
                status: res?.status,
                statusText: res?.statusText,
                clerkTraceId: getTraceId(err, res?.headers)
            };
...
function chunk_HGGLOBDA_parseErrors(data) {
...
// src/api/factory.ts
function chunk_HGGLOBDA_createBackendApiClient(options) {
...
function signedInAuthObject(authenticateContext, sessionToken, sessionClaims) {
    ...
    const apiClient = chunk_HGGLOBDA_createBackendApiClient(authenticateContext);
...
var withDebugHeaders = (requestState)=>{
    ...
    if (requestState.message) {
        try {
            headers.set(chunk_HGGLOBDA_constants.Headers.AuthMessage, requestState.message);
        } catch (e) {}
    }
    if (requestState.reason) {
        try {
            headers.set(chunk_HGGLOBDA_constants.Headers.AuthReason, requestState.reason);
        } catch (e) {}
    }
    if (requestState.status) {
        try {
            headers.set(chunk_HGGLOBDA_constants.Headers.AuthStatus, requestState.status);
        } catch (e) {}
    }
...
var ClerkRequest = class extends Request {
   ...
   */ deriveUrlFromHeaders(req) {
        ...
        const forwardedProto = req.headers.get(chunk_HGGLOBDA_constants.Headers.ForwardedProto);
        const forwardedHost = req.headers.get(chunk_HGGLOBDA_constants.Headers.ForwardedHost);
        const host = req.headers.get(chunk_HGGLOBDA_constants.Headers.Host);
...
var AuthenticateContext = class {
    ...
    initHeaderValues() {
        this.sessionTokenInHeader = this.stripAuthorizationHeader(this.getHeader(chunk_HGGLOBDA_constants.Headers.Authorization));
        this.origin = this.getHeader(chunk_HGGLOBDA_constants.Headers.Origin);
        this.host = this.getHeader(chunk_HGGLOBDA_constants.Headers.Host);
        this.forwardedHost = this.getHeader(chunk_HGGLOBDA_constants.Headers.ForwardedHost);
        this.forwardedProto = this.getHeader(chunk_HGGLOBDA_constants.Headers.CloudFrontForwardedProto) || this.getHeader(chunk_HGGLOBDA_constants.Headers.ForwardedProto);
        this.referrer = this.getHeader(chunk_HGGLOBDA_constants.Headers.Referrer);
        this.userAgent = this.getHeader(chunk_HGGLOBDA_constants.Headers.UserAgent);
        this.secFetchDest = this.getHeader(chunk_HGGLOBDA_constants.Headers.SecFetchDest);
        this.accept = this.getHeader(chunk_HGGLOBDA_constants.Headers.Accept);
    }
    initCookieValues() {
        this.suffixedCookies = this.shouldUseSuffixed();
        this.sessionTokenInCookie = this.getSuffixedOrUnSuffixedCookie(chunk_HGGLOBDA_constants.Cookies.Session);
        this.refreshTokenInCookie = this.getSuffixedCookie(chunk_HGGLOBDA_constants.Cookies.Refresh);
        this.clientUat = Number.parseInt(this.getSuffixedOrUnSuffixedCookie(chunk_HGGLOBDA_constants.Cookies.ClientUat) || "") || 0;
    }
    initHandshakeValues() {
        this.devBrowserToken = this.getQueryParam(chunk_HGGLOBDA_constants.QueryParameters.DevBrowser) || this.getSuffixedOrUnSuffixedCookie(chunk_HGGLOBDA_constants.Cookies.DevBrowser);
        this.handshakeToken = this.getQueryParam(chunk_HGGLOBDA_constants.QueryParameters.Handshake) || this.getCookie(chunk_HGGLOBDA_constants.Cookies.Handshake);
        this.handshakeRedirectLoopCounter = Number(this.getCookie(chunk_HGGLOBDA_constants.Cookies.RedirectCount)) || 0;
    }
    ...
    shouldUseSuffixed() {
        const suffixedClientUat = this.getSuffixedCookie(chunk_HGGLOBDA_constants.Cookies.ClientUat);
        const clientUat = this.getCookie(chunk_HGGLOBDA_constants.Cookies.ClientUat);
        const suffixedSession = this.getSuffixedCookie(chunk_HGGLOBDA_constants.Cookies.Session) || "";
        const session = this.getCookie(chunk_HGGLOBDA_constants.Cookies.Session) || "";
...
async function authenticateRequest(request, options) {
    ...
    function removeDevBrowserFromURL(url) {
        ...
        updatedURL.searchParams.delete(chunk_HGGLOBDA_constants.QueryParameters.DevBrowser);
        updatedURL.searchParams.delete(chunk_HGGLOBDA_constants.QueryParameters.LegacyDevBrowser);
        return updatedURL;
    }
    function buildRedirectToHandshake({ handshakeReason }) {
        ...
        url.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.HandshakeReason, handshakeReason);
        if (authenticateContext.instanceType === "development" && authenticateContext.devBrowserToken) {
            url.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.DevBrowser, authenticateContext.devBrowserToken);
        }
        ...
        return new Headers({
            [chunk_HGGLOBDA_constants.Headers.Location]: url.href
        });
    }
    async function resolveHandshake() {
        ...
        cookiesToSet.forEach((x)=>{
            ...
            if (getCookieName(x).startsWith(chunk_HGGLOBDA_constants.Cookies.Session)) {
                sessionToken = getCookieValue(x);
            }
        });
        if (authenticateContext.instanceType === "development") {
            const newUrl = new URL(authenticateContext.clerkUrl);
            newUrl.searchParams.delete(chunk_HGGLOBDA_constants.QueryParameters.Handshake);
            newUrl.searchParams.delete(chunk_HGGLOBDA_constants.QueryParameters.HandshakeHelp);
            headers.append(chunk_HGGLOBDA_constants.Headers.Location, newUrl.toString());
            headers.set(chunk_HGGLOBDA_constants.Headers.CacheControl, "no-store");
        }
        if (sessionToken === "") {
...
${error.getFullMessage()}`);
    ...
    function handleMaybeHandshakeStatus(authenticateContext2, reason, message, headers) {
        if (isRequestEligibleForHandshake(authenticateContext2)) {
            ...
            if (handshakeHeaders.get(chunk_HGGLOBDA_constants.Headers.Location)) {
                handshakeHeaders.set(chunk_HGGLOBDA_constants.Headers.CacheControl, "no-store");
            }
    ...
    function setHandshakeInfiniteRedirectionLoopHeaders(headers) {
        ...
        const cookieName = chunk_HGGLOBDA_constants.Cookies.RedirectCount;
    ...
    async function authenticateRequestWithTokenInCookie() {
        ...
        const isRequestEligibleForMultiDomainSync = authenticateContext.isSatellite && authenticateContext.secFetchDest === "document" && !authenticateContext.clerkUrl.searchParams.has(chunk_HGGLOBDA_constants.QueryParameters.ClerkSynced);
        ...
        if (authenticateContext.instanceType === "development" && authenticateContext.clerkUrl.searchParams.has(chunk_HGGLOBDA_constants.QueryParameters.DevBrowser)) {
        ...
        if (authenticateContext.instanceType === "development" && isRequestEligibleForMultiDomainSync) {
            ...
            redirectURL.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.ClerkRedirectUrl, authenticateContext.clerkUrl.toString());
            const authErrReason = AuthErrorReason.SatelliteCookieNeedsSyncing;
            redirectURL.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.HandshakeReason, authErrReason);
            const headers = new Headers({
                [chunk_HGGLOBDA_constants.Headers.Location]: redirectURL.toString()
            });
            return handleMaybeHandshakeStatus(authenticateContext, authErrReason, "", headers);
        }
        const redirectUrl = new URL(authenticateContext.clerkUrl).searchParams.get(chunk_HGGLOBDA_constants.QueryParameters.ClerkRedirectUrl);
        if (authenticateContext.instanceType === "development" && !authenticateContext.isSatellite && redirectUrl) {
            const redirectBackToSatelliteUrl = new URL(redirectUrl);
            if (authenticateContext.devBrowserToken) {
                redirectBackToSatelliteUrl.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.DevBrowser, authenticateContext.devBrowserToken);
            }
            redirectBackToSatelliteUrl.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.ClerkSynced, "true");
            const authErrReason = AuthErrorReason.PrimaryRespondsToSyncing;
            redirectBackToSatelliteUrl.searchParams.append(chunk_HGGLOBDA_constants.QueryParameters.HandshakeReason, authErrReason);
            const headers = new Headers({
                [chunk_HGGLOBDA_constants.Headers.Location]: redirectBackToSatelliteUrl.toString()
            });
            return handleMaybeHandshakeStatus(authenticateContext, authErrReason, "", headers);
        }
...
var buildUrl = (_baseUrl, _targetUrl, _returnBackUrl, _devBrowserToken)=>{
    ...
    if (_devBrowserToken && baseUrl.hostname !== res.hostname) {
        res.searchParams.set(chunk_HGGLOBDA_constants.QueryParameters.DevBrowser, _devBrowserToken);
    }
    return res.toString();
};
...
const serverRedirectWithAuth = (clerkRequest, res, opts)=>{
    ...
    const shouldAppendDevBrowser = res.headers.get(chunk_HGGLOBDA_constants.Headers.ClerkRedirectTo) === "true";
...
function debugLogFooter(name) {
    return `[clerk debug end: ${name}] (@clerk/nextjs=${"5.7.5"},next=${package_namespaceObject.i8})`;
}
...
function createClerkClient(options) {
    ...
    const apiClient = chunk_HGGLOBDA_createBackendApiClient(opts);
...
const buildRequestLike = ()=>{
    try {
        const { headers } = __webpack_require__(268);
...
const PUBLISHABLE_KEY = "pk_test_cmVzdGVkLWNvcmdpLTg5LmNsZXJrLmFjY291bnRzLmRldiQ" || 0;
...
function decorateRequest(req, res, requestState, requestData) {
    ...
    if (rewriteURL) {
        ...
        setRequestHeadersOnNextResponse(res, req, {
            [chunk_HGGLOBDA_constants.Headers.AuthStatus]: status,
            [chunk_HGGLOBDA_constants.Headers.AuthToken]: token || "",
            [chunk_HGGLOBDA_constants.Headers.AuthSignature]: token ? createTokenSignature(token, (_a = requestData == null ? void 0 : requestData.secretKey) != null ? _a : SECRET_KEY) : "",
            [chunk_HGGLOBDA_constants.Headers.AuthMessage]: message || "",
            [chunk_HGGLOBDA_constants.Headers.AuthReason]: reason || "",
            [chunk_HGGLOBDA_constants.Headers.ClerkUrl]: req.clerkUrl.toString(),
            ...clerkRequestData ? {
                [chunk_HGGLOBDA_constants.Headers.ClerkRequestData]: clerkRequestData
            } : {}
        });
...
const redirectAdapter = (url)=>{
    return response_NextResponse.redirect(url, {
        headers: {
            [chunk_HGGLOBDA_constants.Headers.ClerkRedirectTo]: "true"
        }
    });
};
...
const clerkClientForRequest = ()=>{
    ...
    try {
        ...
        const encryptedRequestData = getHeader(request, chunk_HGGLOBDA_constants.Headers.ClerkRequestData);
...
const isServerActionRequest = (req)=>{
    var _a, _b;
    return !!req.headers.get(constants_constants.Headers.NextUrl) && (((_a = req.headers.get(chunk_HGGLOBDA_constants.Headers.Accept)) == null ? void 0 : _a.includes("text/x-component")) || ((_b = req.headers.get(chunk_HGGLOBDA_constants.Headers.ContentType)) == null ? void 0 : _b.includes("multipart/form-data")) || !!req.headers.get(constants_constants.Headers.NextAction));
};
const isPageRequest = (req)=>{
    var _a;
    return req.headers.get(chunk_HGGLOBDA_constants.Headers.SecFetchDest) === "document" || req.headers.get(chunk_HGGLOBDA_constants.Headers.SecFetchDest) === "iframe" || ((_a = req.headers.get(chunk_HGGLOBDA_constants.Headers.Accept)) == null ? void 0 : _a.includes("text/html")) || isAppRouterInternalNavigation(req) || isPagesRouterInternalNavigation(req);
};
...
const clerkMiddleware = (...args)=>{
    ...
    return clerkMiddlewareRequestDataStorage.run(clerkMiddlewareRequestDataStore, ()=>{
        const nextMiddleware = withLogger("clerkMiddleware", (logger)=>async (request2, event2)=>{
                ...
                const locationHeader = requestState.headers.get(chunk_HGGLOBDA_constants.Headers.Location);
                if (locationHeader) {
                ...
                if (options.debug) {
                    setRequestHeadersOnNextResponse(handlerResult, clerkRequest, {
                        [chunk_HGGLOBDA_constants.Headers.EnableDebug]: "true"
                    });
                }
...
const handleControlFlowErrors = (e, clerkRequest, requestState)=>{
    switch(e.message){
        case CONTROL_FLOW_ERROR.FORCE_NOT_FOUND:
            return setHeader(response_NextResponse.rewrite(`${clerkRequest.clerkUrl.origin}/clerk_${Date.now()}`), chunk_HGGLOBDA_constants.Headers.AuthReason, "protect-rewrite");
...
(function(root, factory, undef) {
    if (true) {
        // CommonJS
        module.exports = exports = factory(__webpack_require__(31), __webpack_require__(254), __webpack_require__(477), __webpack_require__(612), __webpack_require__(518));
    } else {}
...
(function(root, factory, undef) {
    if (true) {
        // CommonJS
        module.exports = exports = factory(__webpack_require__(31), __webpack_require__(612));
    } else {}
})(void 0, function(CryptoJS) {
    /**
...
(function(root, factory, undef) {
    if (true) {
        // CommonJS
        module.exports = exports = factory(__webpack_require__(31), __webpack_require__(761), __webpack_require__(520));
    } else {}
})(void 0, function(CryptoJS) {
    (function() {
        // Shortcuts
...
var request_cookies = __webpack_require__(249);
// EXTERNAL MODULE: ./node_modules/next/dist/esm/server/web/spec-extension/adapters/headers.js
var headers = __webpack_require__(196);
// EXTERNAL MODULE: ./node_modules/next/dist/esm/server/web/spec-extension/cookies.js
var cookies = __webpack_require__(639);
// EXTERNAL MODULE: ./node_modules/next/dist/esm/client/components/request-async-storage.external.js
var request_async_storage_external = __webpack_require__(192);
...
/* harmony import */ var _reflect__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(868);
...
/* harmony import */ var _cookies__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(639);
/* harmony import */ var _reflect__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(868);
...
const map = __webpack_require__(755);
const { snakeCase } = __webpack_require__(373);
